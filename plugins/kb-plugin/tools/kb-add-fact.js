...
/**
 * KbAddFact Tool - Add facts to knowledge base with validation
 * Restricted to kb-agent use only for secure fact addition
 */
...
const TOOL_METADATA = {
  name: 'KbAddFact',
  description: 'Add a new fact to the knowledge base with topic and source validation',
  parameters: {
    content: {
      type: 'string',
      description: 'The fact content to add',
      required: true
    },
    topics: {
      type: 'string',
...
function validateAndCleanTopics(topicsString) {
  ...
  for (const topic of topics) {
    // Check for malformed topics (the issue we're fixing)
    if (topic.includes('\n') || topic.includes('\r')) {
      throw new Error(`Invalid topic name contains newlines: ${topic.substring(0, 100)}...`);
    }

    // Check for excessive length (likely malformed)
    if (topic.length > 100) {
      throw new Error(`Topic name too long (max 100 chars): ${topic.substring(0, 50)}...`);
    }

    // Check for excessive punctuation (likely malformed content)
    ...
    // Check for sentence-like structure (likely malformed content used as topic)
...
// Execute the add-fact command
async function executeAddFact(content, topics = '', sources = '') {
  return new Promise((resolve, reject) => {
    try {
      ...
      console.error(`Debug: Adding fact with ${cleanedTopics.length} topics: ${cleanedTopics.join(', ')}`);

      const cliPath = findKbCli();

      // Build command arguments
      const cmdArgs = ['add-fact', cleanedContent];
      ...
      child.on('exit', (code) => {
        if (code === 0) {
          ...
          reject(new Error(`Failed to add fact (exit code ${code}): ${stderr}`));
        }
      });

      child.on('error', (err) => {
        reject(new Error(`Failed to execute add-fact: ${err.message}`));
      });
...
async function main() {
  try {
    ...
    if (args.length === 0) {
      console.error('Usage: node kb-add-fact.js <content> [topics] [sources]');
      process.exit(1);
    }
    ...
    const result = await executeAddFact(content, topics, sources);
    console.log(result);
