...
export interface BatchApplyContext {
  sourceIdMap: Map<number, number>; // refId -> actualId
}
...
export class KnowledgeBase {
   ...
   */
  private loadSources(sourcesJsonPath: string): Source[] {
    ...
    try {
      ...
      const sourcesData = JSON.parse(fileContent) as any[];
      return sourcesData.map(sourceObj => Source.fromObject(sourceObj));
    } catch (error) {
   ...
   */
  private migrateOldSourcesIfNeeded(factsJsonPath: string): void {
    if (!fs.existsSync(factsJsonPath)) {
      return;
    }
    ...
    try {
      ...
      const needsMigration = factsData.some(
        (f: any) => Array.isArray(f.sources) && f.sources.length > 0 && typeof f.sources[0] === 'string' && f.sourceIds === undefined
      );
  ...
  createSource(type: 'person' | 'url', title: string, url?: string, addedAt?: string): Source {
    ...
    const source = new Source(id, type, title, url, addedAt);
  ...
  createFact(content: string, topicNames: Set<string>, sourceIds: Set<number>): Fact {
    ...
    const fact = new Fact(id, content, topicNames, sourceIds);
  ...
  loadStagedChanges(): StagedChangesFile | null {
    ...
    try {
      ...
      return JSON.parse(content) as StagedChangesFile;
    } catch (error) {
